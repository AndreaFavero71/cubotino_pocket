...
# This specific script controls two servos based on the movements string from Cubotino_P.py
# 
...
# recovers the gaps between cube_hoder - cube - top_cover, and still getting to a decent cube layers alignment.
#
#############################################################################################################
"""
...
from Cubotino_P_display import display as s_disp    
...
def init_servo(print_out=s_debug, start_pos=0, f_to_close_mode=False):
    ...
    return robot_init_status, timer
...
def read(slow_time=0):
    ...
    if not stop_servos:                          # case there is not a stop request for servos
        if b_servo_stopped==True:                # boolean of bottom servo at location the lifter can be operated
            ...
            time.sleep(slow_time)                # additional time to slow down the robot
...
def flip_to_read(slow_time=0):
    """ Function to raise or lower the top cover to the read position."""
    ...
    if not stop_servos:                            # case there is not a stop request for servos
        if b_servo_stopped==True:                  # boolean of bottom servo at location the lifter can be operated
            ...
            if not flip_to_close_one_step:         # case the flip to close is not set to one step
                ...
                time.sleep(2*t_flip_to_close_time) # time for the servo to reach and rest a bit on the read position
                t_top_cover == 'read'              # variable to track the top cover/lifter position
...
def open_cover(target=0, test=False, slow_time=0):
...
def close_cover(target=0, release=0, timer1=0, timer2=0, test=False, slow_time=0):
...
def spin_out(direction, target=0, release=0, timer1=0, test=False, slow_time=0):
    ...
    if not b_servo_operable and t_top_cover=='read':
        flip_to_open(slow_time=slow_time)
...
def spin_home(slow_time=0):
    ...
    if not b_servo_operable and t_top_cover=='read':
        flip_to_open(slow_time)
    
    if not stop_servos:                         # case there is not a stop request for servos
        if b_servo_operable==False and open_cover!='open':  # variable to block/allow bottom servo operation
            open_cover(slow_time=slow_time)     # top servo is moved to open position
...
def spin(direction, slow_time=0):
...
def rotate_out(direction, slow_time=0):
    ...
    if not stop_servos:                                   # case there is not a stop request for servos
        if t_top_cover!='close':                          # case the top cover is not in close position
            close_cover(slow_time=slow_time)              # top cover is lowered in close position
        ...
        if b_servo_operable==True:                        # variable to block/allow bottom servo operation
            if b_servo_home==True:                        # boolean of bottom servo at home=
                ...
                if t_top_cover=='close':                  # case the top cover is in close position
                    open_cover(slow_time=slow_time)      # top cover is raised in open position
...
def rotate_home(direction, home=0, release=0, timer1=0, test=False, slow_time=0):
...
def servo_solve_cube(moves, scrambling=False, print_out=s_debug, test=False, slow_time=0):
    ...
    if len(moves)>0:                               # case moves > 0 (there are moves)
        if moves[0] == 'S':                        # case the first move requires a cube spin
            flip_to_open(slow_time)                # Top_cover is set to open position
        elif moves[0] == 'R':                      # case the first move requires a cube layer rotation
            flip_to_close(slow_time)               # Top_cover is set to close position
    ...
    for i in range(string_len):                    # iteration over the characters of the moves string
        ...
        if moves[i]=='F':                          # case there is a flip on the move string
            ...
            for f in range(flips):                 # iterates over the number of requested flips        
                ...
                flip_up(slow_time=slow_time)                 # lifter is operated to flip the cube

                if f<(flips-1):                    # case there are further flippings to do
                    flip_to_read(slow_time=slow_time)        # lifter is lowered stopping the top cover in read position (cube not constrained)

                if f==(flips-1) and string_len-(i+2)>0:   # case it's the last flip and there is a following command on the move string
                    if moves[i+2]=='R':            # case the next action is a 1st layer cube rotation
                        flip_to_close(slow_time=slow_time)   # top cover is lowered to close position
                    elif moves[i+2]=='S':          # case the next action is a cube spin
                        flip_to_open(slow_time=slow_time)    # top cover is lowered to open position
            ...
            if b_servo_home==True:                 # case bottom servo is at home
                spin_out(set_dir,slow_time=slow_time)  # call to function to spin the full cube to full CW or CCW
            
            else:                                  # case the bottom servo is at full CW or CCW position
                spin_home(slow_time=slow_time)     # call to function to spin the full cube toward home position
            ...
            if b_servo_home==True:                 # case bottom servo is at home
                rotate_out(set_dir, slow_time=slow_time)  # call to function to rotate cube 1st layer on the set direction, moving out from home
            
            elif b_servo_CW_pos==True:             # case the bottom servo is at full CW position
                if set_dir=='CCW':                 # case the set direction is CCW
                    rotate_home(set_dir, slow_time=slow_time)  # call to function to spin the full cube toward home position
...
def print_info():
    ...
    print(' It is possible to recall the values stored at the Cubotino_P_servo_settings.txt ,')
    ...
    print("\n Enter  'init'     to reload the settings from the last saved Cubotino_P_servo_settings.txt")
...
def test_servos_positions():
    ...
    while True:                                         # infinite loop
        ...
        if ui == "q" or ui == 'Q' or ui == 'exit':      # cases the inputs are for script quitting
            print('Quitting Cubotino_P_servos.py\n\n')  # feedback is printed to terminal
            ...
            t_servo.value = t_servo_open                # top servo is rotated to open position (as defined at the json file)
            parameters = update_parameters()            # parameters for --tune are updated
            print('\nRe-loaded Cubotino_P_servo_settings.txt settings')  # feedback is printed to terminal
            ...
            t_servo.value = t_servo_open                # top servo is rotated to open position (as defined at the json file)
            parameters = update_parameters()            # parameters for --tune are updated
            print('Re-loaded Cubotino_P_servo_settings.txt settings')  # feedback is printed to terminal
...
def test_set_of_movements(slow_time=0):
    ...
    if init_servo(print_out=s_debug, start_pos='read'):    # servos are initialized
        ...
        robot_status, robot_time = servo_solve_cube(movements, print_out=s_debug, test=True, slow_time=slow_time) # robot solver is called
        ...
        if robot_status == 'Cube_solved':                       # case the robot solver returns Cube_solved in the robot_status
            ...
            s_disp.show_on_display('TEST TIME:', str(round(robot_time,1)) + 's', fs1=20, y2=75, fs2=25)  # feedback is printed to the display
...
if __name__ == "__main__":
    ...
    elif args.tune == True:               # case the Cubotino_P_servo.py has been launched with 'tune' argument
    ...
    elif args.slow != None:               # case the Cubotino_P_servo.py has been launched with 'slow' argument
