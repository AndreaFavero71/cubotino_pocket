...
"""
######################################################################################################################
# Andrea Favero 11 Febraury 2024
# 
...
from Cubotino_P_display import display as disp           # custom library controlling Cubotino disply module
from Cubotino_P import tune_image_setup as camera_setup  # import the camera setting up function
from Cubotino_P import read_camera as read_camera        # import the camera reading function
from Cubotino_P import frame_cropping as crop            # import the image cropping function
from Cubotino_P import warp_image as warp                # import the image warping function
from Cubotino_P import close_camera as close_cam         # import the close_camera function
from Cubotino_P import text_font as text_font            # import the font for cv2 function
...
def read_servo_settings_file(fname=''):
    ...
    if len(fname) == 0:                                              # case fname equals empty string
        fname = 'Cubotino_P_servo_settings.txt'                      # fname for the text file to retrieve servos settings
    ...
    if os.path.exists(fname):                                        # case the servo_settings file exists
        ...
        srv_settings = update_srv_settings_file(fname, srv_settings) # eventual new servo settings are appended to the older ones
...
def read_cam_settings_file(fname=''):
    ...
    from Cubotino_P_update_settings_file import update_settings_file
    import os.path, pathlib, json                              # libraries needed for the json, and parameter import
    # convenient choice for Andrea Favero, to upload the settings fitting his robot based on mac address check                
    from getmac import get_mac_address                         # library to get the device MAC ddress
    ...
    if os.path.exists(fname):                                  # case the settings file exists
        ...
        # update key-values parameters, needed in case of additional parameters added at remote repo
        cam_settings = update_settings_file(fname, cam_settings)   # eventual new settings are appended to the previous
...
# Expo shift
s_expo_shift = tk.Scale(expo_label, label="EXPOSURE SHIFT", font=('arial','14'), orient='horizontal',
