...
class Settings:
    ...
    def check_local_settings(self):
        ...
        fnames_default = ('Cubotino_P_settings_default.txt' , 'Cubotino_P_servo_settings_default.txt')
        fnames_local = ('Cubotino_P_settings.txt', 'Cubotino_P_servo_settings.txt')
    ...
    def get_settings_fname(self):
        fname = 'Cubotino_P_settings.txt'                         # file name with general settings
        if self.eth_mac in self.macs_AF:                          # case the script is running on AF (Andrea Favero) robot
            pos = self.macs_AF.index(self.eth_mac)                # mac address row position in macs_AF.txt file
    ...
    def get_servo_settings_fname(self):
        fname = 'Cubotino_P_servo_settings.txt'                   # file name with servos settings
        if self.eth_mac in self.macs_AF:                          # case the script is running on AF (Andrea Favero) robot
            pos = self.macs_AF.index(self.eth_mac)                # mac address row position in macs_AF.txt file
            fname = self.get_fname_AF(fname, pos)                 # AF robot s file name (do not use these at the start)
        else:                                                     # case the script is not running on AF (Andrea Favero) robot
    ...
    def read_settings(self, fname=''):
        """Function reading the Cubotino_P_settings.txt file. Retrieves a dict with parameters and settings."""
        ...
        if os.path.exists(fname):                                 # case the settings file exists
            ...
            backup_fname = 'Cubotino_P_settings_backup.txt'       # filename for the backup
    ...
    def read_servos_settings(self, fname=''):
        """Function reading the Cubotino_P_servo_settings.txt . Retrieves a dict with parameters and settings."""
        ...
        if os.path.exists(fname):                                 # case the servo_settings file exists
            ...
            backup_fname = 'Cubotino_P_servo_settings_backup.txt' # filename for the backup
...
#             return servo_s                                                      # return servos settings dict
    ...
    def save_setting(self, fname, data, debug=False):
        ...
        if debug:                                                # case debug variable is set true
            ...
            for parameter, setting in data.items():              # iteration over the settings dict
                print(f"{parameter}: {setting}")                  # feedback is printed to the terminal
    ...
    def update_settings_file(self, fname, s):
        """Function to check if the existing fname (Cubotino_P_settings.txt) has all
        the parameters that were instroduced after the first release.
        Cubotino_P_settings.txt file will be updated with eventually missed parameters
        ...
        if any_change:
            ...
            print('Action necessary for compatibility with the latest downloaded Cubotino_P.py \n')
    ...
    def update_servos_settings_file(self, fname, servo_s, json):
        """Function to check if the existing setting file(fname: Cubotino_P_servos_settings.txt)
            has all the parameters that were introduced after first release.
            Cubotino_P_servos_settings.txt will be eventually updated with missed parameters
            and related default values.
            """
